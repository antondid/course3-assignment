...
train_data <- cbind(train_labels, train_sub, train_full[, mean_std == TRUE])
...
test_data <- cbind(test_labels, test_sub, test_full[, mean_std == TRUE])


# merging datasets together to make the big one
merged_data <- merge(activity, rbind(train_data, test_data), by = "activity-id")[,-1]
